...
def sample(trial_num):
    ...
    Y = (np.outer(alpha, nones) + B @ X.T + Z) # Beta implicitly embeds correlated factor returns already here
    m = Y @ nones / n
    Y = Y @ Pn # center
...
def compute(Y, X, B, svar, m):
    ...
    vals = vals[::-1] 
    vecs = np.fliplr(vecs)
    sH = Y @ vecs / np.sqrt(p * vals)  #  Convert dual eigenvectors (vecs) → asset-space eigenvectors (sH).
    eigs = vals * p / n
    ...
    #print(np.mean(Delta))
    # The PCA method here shrunk eigenvalues (Ψ² λᵢ), and estimated residuals (Δ, scaled to γ²)
